...
HYBRID_SAFETY_REFINE_ENABLE = os.environ.get(
    "HYBRID_SAFETY_REFINE_ENABLE", "false"
).lower() in {"1", "true", "yes", "on"}
HYBRID_SAFETY_SKIP_AGGRESSION = os.environ.get(
    "HYBRID_SAFETY_SKIP_AGGRESSION", "true"
).lower() in {"1", "true", "yes", "on"}
HYBRID_SAFETY_POST_BOMB_COMMIT_STEPS = int(os.environ.get(
    "HYBRID_SAFETY_POST_BOMB_COMMIT_STEPS", "6"
))
HYBRID_SAFETY_LOOKAHEAD = int(os.environ.get("HYBRID_SAFETY_LOOKAHEAD", "8"))
...
def _safe_refine_action_score(action, pos, my_pos, game_map, bomb_set,
                              danger_time, own_bomb_pos=None):
    if action == A_BOMB:
        return -1e9
    if pos is None:
        return -1e9
    r, c = pos
    if not _is_passable(r, c, game_map, bomb_set) and pos != my_pos:
        return -1e9
    if danger_time[r, c] <= 2:
        return -1e9
    ...
    score = area * 20.0
    if danger_time[r, c] < INF:
        score += min(danger_time[r, c], HYBRID_SAFETY_LOOKAHEAD) * 8.0
    else:
        score += 120.0
    ...
    if own_bomb_pos is not None:
        before = abs(my_pos[0] - own_bomb_pos[0]) + abs(my_pos[1] - own_bomb_pos[1])
        after = abs(pos[0] - own_bomb_pos[0]) + abs(pos[1] - own_bomb_pos[1])
        score += (after - before) * 35.0

    if action == A_STOP and danger_time[my_pos[0], my_pos[1]] <= HYBRID_SAFETY_LOOKAHEAD:
        score -= 250.0
    return score
...
class Agent:
    ...
    def __init__(self, agent_id: int):
        ...
        self.last_own_bomb_pos = None
        self.safety_refine_counters = {
            "safety_refine_active": 0,
            "post_bomb_escape_override": 0,
            "blastline_stop_prevented": 0,
            "return_to_blastline_prevented": 0,
            "enemy_bomb_avoidance_override": 0,
            "fallback_to_production": 0,
        }
    ...
    def _select_safety_override(self, production_action, final_scores, action_positions,
                                my_pos, game_map, bomb_set, danger_time,
                                own_bomb_pos=None):
        baseline_pos = action_positions.get(production_action, my_pos)
        baseline_score = _safe_refine_action_score(
            production_action, baseline_pos, my_pos, game_map, bomb_set,
            danger_time, own_bomb_pos
        )
        best_action = production_action
        best_score = baseline_score
        for action in [A_STOP] + MOVE_ACTIONS:
            if action == A_STOP and danger_time[my_pos[0], my_pos[1]] <= HYBRID_SAFETY_LOOKAHEAD:
                continue
            if final_scores.get(action, -1e9) <= -1e8:
                continue
            pos = action_positions.get(action, my_pos)
            score = _safe_refine_action_score(
                action, pos, my_pos, game_map, bomb_set, danger_time, own_bomb_pos
            )
    ...
    def _maybe_safety_refine(self, obs, production_action, final_scores,
                             action_positions, my_pos, game_map, bomb_set,
                             danger_time, emergency_first_action):
        if not HYBRID_SAFETY_REFINE_ENABLE:
            return production_action
        if emergency_first_action is not None and production_action == emergency_first_action:
            self.safety_refine_counters["fallback_to_production"] += 1
            return production_action

        step = int(obs["step"]) if "step" in obs else None
        recent_own_bomb = (
            step is not None
            and self.last_own_bomb_pos is not None
            and step - self.last_bomb_step <= HYBRID_SAFETY_POST_BOMB_COMMIT_STEPS
        )
        prod_pos = action_positions.get(production_action, my_pos)
        current_danger = danger_time[my_pos[0], my_pos[1]] <= HYBRID_SAFETY_LOOKAHEAD
        prod_danger = danger_time[prod_pos[0], prod_pos[1]] <= HYBRID_SAFETY_LOOKAHEAD
        low_area = _reachable_safe_area(my_pos, game_map, bomb_set, danger_time, max_depth=5) <= 3
        stop_in_blast = production_action == A_STOP and current_danger

        own_blast_return = False
        if recent_own_bomb:
            radius = 1 + int(obs["players"][self.agent_id][4])
            own_blast = _blast_cells(self.last_own_bomb_pos[0], self.last_own_bomb_pos[1], radius, game_map)
            own_blast_return = prod_pos in own_blast or my_pos in own_blast

        enemy_bomb_near = False
        bombs_arr = np.asarray(obs["bombs"], dtype=np.int32)
        if bombs_arr.size:
            if bombs_arr.ndim == 1:
                bombs_arr = bombs_arr.reshape(1, -1)
            for bomb in bombs_arr:
                owner = int(bomb[3]) if len(bomb) > 3 else -1
                timer = int(bomb[2]) if len(bomb) > 2 else INF
                if owner != self.agent_id and timer <= HYBRID_SAFETY_LOOKAHEAD:
                    cells = _blast_cells(int(bomb[0]), int(bomb[1]), 1 + int(obs["players"][owner][4]) if 0 <= owner < len(obs["players"]) else 1, game_map)
                    if my_pos in cells or prod_pos in cells:
                        enemy_bomb_near = True
                        break

        if not (current_danger or prod_danger or low_area or stop_in_blast or own_blast_return or enemy_bomb_near):
            self.safety_refine_counters["fallback_to_production"] += 1
            return production_action

        self.safety_refine_counters["safety_refine_active"] += 1
        override = self._select_safety_override(
            production_action, final_scores, action_positions, my_pos,
            game_map, bomb_set, danger_time,
            self.last_own_bomb_pos if recent_own_bomb else None,
        )
        if override == production_action:
            self.safety_refine_counters["fallback_to_production"] += 1
            return production_action

        if stop_in_blast:
            self.safety_refine_counters["blastline_stop_prevented"] += 1
        elif own_blast_return:
            self.safety_refine_counters["return_to_blastline_prevented"] += 1
        elif recent_own_bomb:
            self.safety_refine_counters["post_bomb_escape_override"] += 1
        elif enemy_bomb_near:
            self.safety_refine_counters["enemy_bomb_avoidance_override"] += 1
        return override
    ...
    def act(self, obs: dict) -> int:
        ...
        best_action = self._maybe_safety_refine(
            obs,
            production_action,
            final_scores,
            action_positions,
