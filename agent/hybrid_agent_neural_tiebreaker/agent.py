"""
Hybrid Bomberland Agent: Rule-based + BFS + Heuristic Scoring + Neural Tiebreaker.

Uses neural ranker only when heuristic top actions are very close (within
TIE_MARGIN), instead of adding neural bonus globally.
...
Architecture:
  ...
  4. Heuristic Scorer   — rank all 6 actions
  5. Neural Tiebreaker  — if top heuristic scores are close, use neural prior
                          to pick among candidates (research-only, off by default)
"""
...
# Neural tiebreaker constants
TIE_MARGIN = 5
TIE_TOP_K = 2
MIN_NEURAL_CONFIDENCE_GAP = 0.08
NEURAL_TIEBREAKER_CHECKPOINT = "ml/checkpoints/action_ranker_bomb_fixed.pt"
...
# ==============================================================================
# Neural Prior (tiebreaker-only, never affects safety)
# ==============================================================================

class NeuralPrior:
    """Lightweight neural ranker for tiebreaking only.

    Loads a pre-trained SimpleCNNPolicy checkpoint. If torch or the checkpoint
    is unavailable, degrades gracefully (available=False).
    """
    ...
    def rank_candidates(self, obs, agent_id, candidates):
        """Return (best_action, confidence_gap) for a set of candidate actions.

        confidence_gap = prob[best] - prob[second_best] among candidates.
        Returns (None, 0.0) if unavailable or on error.
        """
        ...
        try:
            ...
            x = encode_observation(obs, agent_id)
            x_tensor = self.torch.from_numpy(x).unsqueeze(0)
            with self.torch.no_grad():
                logits = self.model(x_tensor).squeeze(0)

            mask = self.torch.zeros(6, dtype=self.torch.bool)
            for a in candidates:
                mask[a] = True

            masked_logits = logits.masked_fill(~mask, -1e9)
            probs = self.torch.softmax(masked_logits, dim=0)
...
class Agent:
    ...
    Hybrid Bomberland agent combining:
      ...
      - Neural tiebreaker: when top heuristic scores are close, use neural
        prior to pick among candidates (research-only, off by default)

    No ML, no network, no disk I/O.  Under 100 ms per act().
    """
    team_id = "HybridAgent-NeuralTiebreaker"

    def __init__(self, agent_id: int):
        self.agent_id = int(agent_id)
        self.position_history = []
        self.max_position_history = MAX_POSITION_HISTORY
        self.neural = NeuralPrior(NEURAL_TIEBREAKER_CHECKPOINT)
        self._neural_activations = 0
    ...
    def act(self, obs: dict) -> int:
        ...
        # ---- 5b. Advantage weighting (same as online_robust) ----
        ...
        # ---- 5c. Neural tiebreaker ----
        self._neural_calls += 1
        tiebreaker_activated = False

        if self.neural.available:
            # Sort valid actions by final_score descending
            valid = [(a, s) for a, s in final_scores.items() if s > -1e8]
            valid.sort(key=lambda x: x[1], reverse=True)

            if len(valid) >= 2:
                heuristic_best = valid[0][0]
                top_score = valid[0][1]
                second_score = valid[1][1]
                if top_score - second_score <= TIE_MARGIN:
                    tiebreaker_activated = True
                    self._neural_activations += 1
                    # Only consider top 2 heuristic actions
                    candidates = [valid[0][0], valid[1][0]]
                    neural_choice, gap = self.neural.rank_candidates(
                        obs, self.agent_id, candidates
                    )
                    if (
                        neural_choice is not None
                        and neural_choice in candidates
                        and gap >= MIN_NEURAL_CONFIDENCE_GAP
                        and neural_choice != heuristic_best
                    ):
                        self._neural_interventions += 1
                        # Apply decisive bonus to neural choice
                        final_scores[neural_choice] = (
                            final_scores[neural_choice] + TIE_MARGIN + 1.0
                        )

        # ---- 5d. Select best action ----
        best_action = A_STOP
        best_score = -1e9
