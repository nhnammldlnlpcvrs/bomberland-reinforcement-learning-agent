...
# ==============================================================================
# Optional PPO tie-breaker config
# ==============================================================================
#
# This experimental agent must behave like online_robust by default.  The PPO
# path is therefore opt-in and guarded by the rule scorer's safety checks.

ENABLE_RL_TIEBREAKER = os.environ.get("HYBRID_RULE_RL_ENABLE", "false").lower() in (
    "1",
    "true",
    "yes",
    "on",
)
RL_CHECKPOINT_PATH = os.environ.get(
    "HYBRID_RULE_RL_CHECKPOINT",
    str(Path(__file__).resolve().parents[2] / "ml" / "checkpoints" / "rl_agent_pure" / "ppo_validation_gated_distill_best.zip"),
)
RL_MAX_OVERRIDE_CONFIDENCE = float(os.environ.get("HYBRID_RULE_RL_MAX_CONFIDENCE", "0.95"))
RL_ONLY_SAFE_ACTIONS = os.environ.get("HYBRID_RULE_RL_ONLY_SAFE_ACTIONS", os.environ.get("HYBRID_RULE_RL_SAFE_ONLY", "true")).lower() in (
    "1",
    "true",
    "yes",
    "on",
)
RL_MIN_STEP = int(os.environ.get("HYBRID_RULE_RL_MIN_STEP", "30"))
RL_TIE_SCORE_MARGIN = float(os.environ.get("HYBRID_RULE_RL_MAX_RULE_GAP", os.environ.get("HYBRID_RULE_RL_TIE_SCORE_MARGIN", "35.0")))
RL_MIN_PPO_MARGIN = float(os.environ.get("HYBRID_RULE_RL_MIN_PPO_MARGIN", "0.08"))
RL_MIN_CANDIDATES = int(os.environ.get("HYBRID_RULE_RL_MIN_CANDIDATES", "2"))
RL_MAX_CANDIDATES = int(os.environ.get("HYBRID_RULE_RL_MAX_CANDIDATES", "4"))
RL_SAFE_DANGER_MIN = int(os.environ.get("HYBRID_RULE_RL_SAFE_DANGER_MIN", "4"))
...
# ==============================================================================
# Optional RL tie-breaker
# ==============================================================================

class _RLTiebreaker:
    """Lazy PPO action scorer used only behind rule-policy safety gates."""
    ...
    def _try_load(self):
        try:
            path = Path(RL_CHECKPOINT_PATH)
            if not path.exists():
                self.load_error = f"missing_checkpoint:{path}"
                return
    ...
    def _encode(self, obs):
        game_map = np.asarray(obs["map"], dtype=np.int16)
        players = np.asarray(obs["players"], dtype=np.int16)
        bombs = np.asarray(obs["bombs"], dtype=np.int16)
        if bombs.size == 0:
            bombs = np.zeros((0, 4), dtype=np.int16)
        elif bombs.ndim == 1:
            bombs = bombs.reshape(1, -1)

        planes = np.zeros((19, BOARD_SIZE, BOARD_SIZE), dtype=np.float32)
        planes[0] = game_map == TILE_WALL
        planes[1] = game_map == TILE_BOX
        planes[2] = game_map == TILE_GRASS
        planes[3] = game_map == TILE_RADIUS
        planes[4] = game_map == TILE_CAPACITY

        my_r, my_c, alive, _bombs_left, _bonus = _my_state(obs, self.agent_id)
        if alive and 0 <= my_r < BOARD_SIZE and 0 <= my_c < BOARD_SIZE:
            planes[5, my_r, my_c] = 1.0
        ...
        for idx, player in enumerate(players):
            if idx == self.agent_id or int(player[2]) != 1:
                continue
            alive_enemies += 1
            er, ec = int(player[0]), int(player[1])
        ...
        for bomb in bombs:
            ...
            if 0 <= br < BOARD_SIZE and 0 <= bc < BOARD_SIZE:
                planes[8, br, bc] = 1.0
                planes[9, br, bc] = max(0.0, min(1.0, timer / BOMB_TIMER))
                if owner == self.agent_id:
                    planes[10, br, bc] = 1.0
                else:
                    planes[11, br, bc] = 1.0

        danger = _compute_danger_map(game_map, players, bombs)
        planes[12] = danger <= 1
        planes[13] = danger <= 3
        planes[14] = danger <= BOMB_TIMER
        planes[15] = self._reachable_plane(game_map, _bomb_set(bombs), danger, (my_r, my_c))
        planes[16] = self._legal_move_cells(obs, game_map, bombs, my_r, my_c)
        ...
        planes[18].fill(max(0.0, min(1.0, int(obs.get("step", 0) or 0) / 500.0)))
        return planes

    def _reachable_plane(self, game_map, bomb_set, danger, start, max_depth=12):
        out = np.zeros((BOARD_SIZE, BOARD_SIZE), dtype=np.float32)
        if not _in_bounds(start[0], start[1]):
            return out
        ...
        while q:
            ...
            for action in MOVE_ACTIONS:
                ...
                nr, nc = r + dr, c + dc
                npos = (nr, nc)
                if npos in seen or not _is_passable(nr, nc, game_map, bomb_set):
                    continue
                seen.add(npos)
                q.append((nr, nc, dist + 1))
        return out

    def _legal_move_cells(self, obs, game_map, bombs, my_r, my_c):
        cells = np.zeros((BOARD_SIZE, BOARD_SIZE), dtype=np.float32)
        bomb_set = _bomb_set(bombs)
        if 0 <= my_r < BOARD_SIZE and 0 <= my_c < BOARD_SIZE:
            cells[my_r, my_c] = 1.0
        for action in MOVE_ACTIONS:
            dr, dc = DIRS[action]
    ...
    def action_scores(self, obs):
        ...
        try:
            ...
            with torch.no_grad():
                tensor = torch.as_tensor(encoded[None], dtype=torch.float32, device=self.device)
...
class Agent:
    ...
    def __init__(self, agent_id: int):
        ...
        self.rl_tiebreaker = _RLTiebreaker(self.agent_id) if ENABLE_RL_TIEBREAKER else None
        self.debug_counters = {
            "rl_loaded": int(bool(self.rl_tiebreaker and self.rl_tiebreaker.loaded)),
            "rl_inference_errors": 0,
            "rl_tiebreaker_used": 0,
            "rl_action_accepted": 0,
            "rl_action_rejected_by_safety": 0,
        }
    ...
    def _safe_tie_candidates(self, final_scores, best_action, best_score):
        """Return only rule-valid actions close enough to be true tie-breaks."""
        candidates = []
        for action, score in final_scores.items():
            if score <= -1e8:
                continue
            if best_score - score <= RL_TIE_SCORE_MARGIN:
                # Bombs remain rule-owned.  RL may rank a bomb only when the
                # rule policy already selected a bomb after escape/value checks.
                if action == A_BOMB and best_action != A_BOMB:
                    continue
                candidates.append(action)
        return candidates

    def _apply_rl_tiebreaker(self, obs, candidates, best_action):
        if not ENABLE_RL_TIEBREAKER or not self.rl_tiebreaker or not self.rl_tiebreaker.loaded:
            return best_action
        if len(candidates) < 2:
            return best_action
        try:
            probs = self.rl_tiebreaker.action_scores(obs)
        except Exception:
            self.debug_counters["rl_inference_errors"] += 1
            return best_action
        if probs is None or len(probs) < len(ALL_ACTIONS):
            self.debug_counters["rl_inference_errors"] += 1
            return best_action

        self.debug_counters["rl_tiebreaker_used"] += 1
        chosen = max(candidates, key=lambda action: float(probs[action]))

        # OOD guard: very high confidence from an auxiliary PPO is treated as
        # suspicious because the rule policy is the safety owner in this branch.
        if float(probs[chosen]) > RL_MAX_OVERRIDE_CONFIDENCE:
            self.debug_counters["rl_action_rejected_by_safety"] += 1
            return best_action

        if RL_ONLY_SAFE_ACTIONS and chosen not in candidates:
            self.debug_counters["rl_action_rejected_by_safety"] += 1
            return best_action

        self.debug_counters["rl_action_accepted"] += int(chosen != best_action)
        return int(chosen)
    ...
    def act(self, obs: dict) -> int:
        ...
        if ENABLE_RL_TIEBREAKER:
            candidates = self._safe_tie_candidates(final_scores, best_action, best_score)
            best_action = self._apply_rl_tiebreaker(obs, candidates, best_action)
