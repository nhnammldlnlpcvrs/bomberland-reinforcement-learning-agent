...
# Endgame aggression multipliers (applied when enemies_alive == 1)
ENDGAME_ENEMY_APPROACH_BASE = 130
ENDGAME_ENEMY_APPROACH_DECAY = 10
ENDGAME_ESCAPE_PRESSURE_MULTIPLIER = 0.55
ENDGAME_BOX_FARM_BASE = 70
ENDGAME_BOX_FARM_DECAY = 6
ENDGAME_BOX_ADJACENCY = 50
ENDGAME_NEAR_ENEMY_PRESSURE_2 = 180
ENDGAME_NEAR_ENEMY_PRESSURE_4 = 90
ENDGAME_TERRITORY_PRESSURE_MULTIPLIER = 0.85
...
def _nearby_enemy_pressure(pos, players, agent_id, endgame=False):
    """Small risk estimate for being close to alive enemies."""
    p2 = ENDGAME_NEAR_ENEMY_PRESSURE_2 if endgame else NEAR_ENEMY_PRESSURE_2
    p4 = ENDGAME_NEAR_ENEMY_PRESSURE_4 if endgame else NEAR_ENEMY_PRESSURE_4
    pressure = 0
    for i, p in enumerate(players):
...
def _enemy_vulnerability_score(enemy_pos, game_map, bomb_set, danger_time,
                                players, agent_id):
    """Cheap vulnerability estimate using only local features (no BFS)."""
    er, ec = enemy_pos

    # Find enemy stats from players array
    enemy_bombs_left = 1
    enemy_radius_bonus = 0
    for i, p in enumerate(players):
        if i != agent_id and int(p[2]) == 1:
            if int(p[0]) == er and int(p[1]) == ec:
                enemy_bombs_left = int(p[3])
                enemy_radius_bonus = int(p[4])
                break

    score = 0

    # Mobility: fewer free neighbors = more trapped
    free = _free_neighbors(enemy_pos, game_map, bomb_set)
    score += max(0, 4 - free) * 50

    # Zone type
    zone = _is_corridor_or_dead_end(enemy_pos, game_map, bomb_set)
    if zone == "dead_end":
        score += 120
    elif zone == "corridor":
        score += 50

    # No bombs = can't bomb-escape
    if enemy_bombs_left == 0:
        score += 80

    # High-radius enemies are dangerous
    score -= enemy_radius_bonus * 35

    # Enemy in danger zone = vulnerable
    enemy_danger = danger_time[er, ec]
    if enemy_danger <= 3:
        score += 120
    elif enemy_danger <= 5:
        score += 40
...
def _can_escape_after_bomb_at(obs, agent_id, game_map, bomb_set, bomb_r, bomb_c):
    """Check escape path exists from (bomb_r, bomb_c) after placing a bomb there."""
    players = obs["players"]

    bombs_arr = np.asarray(obs["bombs"], dtype=np.int32)
    if bombs_arr.size == 0:
        new_bombs = np.array([[bomb_r, bomb_c, BOMB_TIMER, agent_id]], dtype=np.int32)
    else:
        if bombs_arr.ndim == 1:
            bombs_arr = bombs_arr.reshape(1, -1)
        new_bomb_row = np.array([[bomb_r, bomb_c, BOMB_TIMER, agent_id]], dtype=np.int32)
        new_bombs = np.vstack([bombs_arr, new_bomb_row])

    new_danger = _compute_danger_map(game_map, players, new_bombs)
    result = _find_nearest_safe(
        (bomb_r, bomb_c), game_map, bomb_set, new_danger, MAX_BFS_ESCAPE
    )
    return result is not None


def _score_bomb_spot(r, c, obs, agent_id, game_map, bomb_set, danger_time,
                     enemies, bonus):
    """Score a hypothetical bomb at cell (r,c). Returns -1e9 if invalid."""
    if (r, c) in bomb_set:
        return -1e9
    if not _can_escape_after_bomb_at(obs, agent_id, game_map, bomb_set, r, c):
        return -1e9
    ...
    escape_pressure = _estimate_enemy_escape_pressure(obs, bomb_pos, agent_id)
    if not _is_meaningful_bomb(obs, bomb_pos, agent_id, escape_pressure):
        return -1e9

    boxes = _boxes_in_blast(r, c, radius, game_map)
    threatens = _enemy_in_blast_line(r, c, radius, game_map, players, agent_id)
    ...
    simulated_danger = _simulate_bomb_danger_map(obs, bomb_pos, agent_id, BOMB_TIMER)
...
def _best_bomb_spot(obs, agent_id, game_map, bomb_set, danger_time, max_depth=3):
    """BFS to find the best reachable cell for placing a bomb. Returns (pos, first_action, score)."""
    my_r, my_c, alive, bombs_left, bonus = _my_state(obs, agent_id)
    if bombs_left <= 0:
    ...
    while q:
        ...
        if danger_time[r, c] <= 1:
            if dist > 0:
                continue

        if dist > 0:
            spot_score = _score_bomb_spot(
                r, c, obs, agent_id, game_map, bomb_set, danger_time,
                enemies, bonus
            )
    ...
    return best_spot, best_first_action, best_score
...
def _score_move(action, my_pos, my_state, game_map, players, bomb_set,
                ...
                position_history=None, bomb_spot_info=None):
    ...
    endgame = num_enemies == 1
    enemy_pressure = _nearby_enemy_pressure(npos, players, agent_id, endgame)
    if zone_type == "dead_end":
    ...
    tp_multiplier = ENDGAME_TERRITORY_PRESSURE_MULTIPLIER if endgame else TERRITORY_PRESSURE_MULTIPLIER
    score += territory_pressure * tp_multiplier
    ...
    # ----- Box-farm approach -----
    if box_spots:
        box_base = ENDGAME_BOX_FARM_BASE if endgame else 100
        box_decay = ENDGAME_BOX_FARM_DECAY if endgame else 8
        ...
        if box_bfs is not None:
            ...
            score += max(0, box_base - box_decay * box_dist)

    # Near-box adjacency bonus (good position for future bombing)
    if bombs_left > 0:
        box_adj = ENDGAME_BOX_ADJACENCY if endgame else 80
    ...
    # ----- Enemy approach (with vulnerability bonus) -----
    if enemies:
        ...
        if enemy_bfs is not None:
            _, enemy_dist, target_enemy = enemy_bfs
            vuln = _enemy_vulnerability_score(
                target_enemy, game_map, bomb_set, danger_time,
                players, agent_id
            )
            vuln_bonus = min(90, vuln * 0.45)
            if endgame:
                base = ENDGAME_ENEMY_APPROACH_BASE + vuln_bonus
                decay = ENDGAME_ENEMY_APPROACH_DECAY
            else:
                base = 80 + vuln_bonus
                decay = 6
...
def _score_bomb(my_pos, my_state, game_map, players, bomb_set,
    ...
    enemies = _alive_enemies(obs, agent_id)
    ep_multiplier = ENDGAME_ESCAPE_PRESSURE_MULTIPLIER if len(enemies) == 1 else ESCAPE_PRESSURE_SCORE_MULTIPLIER
    score += escape_pressure * ep_multiplier
...
class Agent:
    ...
    def act(self, obs: dict) -> int:
        ...
        # ---- 3b. Best bomb spot (Phase C) ----
        best_spot, best_spot_action, best_spot_score = _best_bomb_spot(
            obs, self.agent_id, game_map, bomb_set, danger_time, max_depth=3
        )
        bomb_spot_info = (best_spot, best_spot_action, best_spot_score)
        ...
        for action in ALL_ACTIONS:
            if action == A_BOMB:
                ...
                score, pos = _score_move(action, my_pos, my_state, game_map,
                                         ...
                                         self.position_history, bomb_spot_info)
