...
AGENTS = {
    "hybrid_safety_refined": "agent/hybrid_agent_safety_refined",
...
class CompactStats:
    ...
    safety_refine_counters: dict[str, int] = field(default_factory=dict)
    safety_refine_reject_reason_counts: dict[str, int] = field(default_factory=dict)


def _set_env(safety_enabled: bool) -> None:
    os.environ["HYBRID_MODEL_ENABLE"] = "true"
    os.environ["HYBRID_MODEL_MAX_LATENCY_MS"] = "5"
    os.environ["HYBRID_SAFETY_REFINE_ENABLE"] = "true" if safety_enabled else "false"
    os.environ["HYBRID_SAFETY_SKIP_AGGRESSION"] = "true"
    os.environ["HYBRID_SAFETY_POST_BOMB_COMMIT_STEPS"] = "6"
    os.environ["HYBRID_SAFETY_LOOKAHEAD"] = "8"
...
def _reset_agent_state(agent) -> None:
    ...
    if hasattr(agent, "safety_refine_counters"):
        agent.safety_refine_counters = {
    ...
    if hasattr(agent, "safety_refine_reject_reason_counts"):
        agent.safety_refine_reject_reason_counts = {}
...
def disabled_parity_check(seeds: list[int], max_steps: int) -> dict:
    ...
    for seed in seeds:
        ...
        candidate_agents = [_load_agent("hybrid_safety_refined", idx) for idx in range(4)]
...
def _roster(seed: int) -> list[str]:
    roster = ["hybrid_safety_refined", "hybrid_safety_refined", "production", "production"]
...
def _run_episode(seed: int, max_steps: int, timeout_s: float, stats: dict[str, CompactStats], cache: dict[tuple[str, int], object]) -> None:
    ...
    for idx, agent in enumerate(agents):
        if roster[idx] != "hybrid_safety_refined":
            continue
        _merge_counter(stats["hybrid_safety_refined"].safety_refine_counters, getattr(agent, "safety_refine_counters", {}))
        _merge_counter(stats["hybrid_safety_refined"].safety_refine_reject_reason_counts, getattr(agent, "safety_refine_reject_reason_counts", {}))
...
def _summary_row(row: CompactStats) -> dict:
    ...
    return {
        ...
        "safety_refine_counters": dict(row.safety_refine_counters),
        "safety_refine_reject_reason_counts": dict(row.safety_refine_reject_reason_counts),
    }
...
def _verdict(summary: dict[str, dict], parity: dict) -> tuple[str, list[str]]:
    cand = summary["hybrid_safety_refined"]
    ...
    counters = cand.get("safety_refine_counters", {})
    if int(counters.get("safety_refine_active", 0)) <= 0:
        reasons.append("safety refine logic did not activate")
    if reasons:
        return "REJECT_PROMOTION", reasons
    return "PROMOTE_CANDIDATE", ["candidate passes safety refinement gate"]


def _write_report(path: Path, payload: dict) -> None:
    summary = payload["summary"]
    lines = [
        "# Hybrid Safety Refined Report", "",
        f"Generated: {payload['generated']}", "",
        "## Scope", "",
        "- Candidate: `agent/hybrid_agent_safety_refined/`",
    ...
    counters = summary["hybrid_safety_refined"]["safety_refine_counters"]
    rejects = summary["hybrid_safety_refined"]["safety_refine_reject_reason_counts"]
    lines.extend(["", "## Safety Refine Counters", ""])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark hybrid safety refined candidate.")
    ...
    parser.add_argument("--json_output", default="logs/hybrid_safety_refined_benchmark.json")
    parser.add_argument("--report", default="docs/HYBRID_SAFETY_REFINED_REPORT.md")
